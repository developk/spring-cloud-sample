    ...
    >
      ...
      <v-toolbar-title v-if="config.toolbarTitle">
        <v-img
          ...
          width="98"
        />
        <div v-if="config.systemTitle" class="system-title">
          {{ config.systemTitle }}
        </div>
      </v-toolbar-title>
      ...
      >
        <template v-slot:activator="{ on, attrs }">
          <v-btn v-bind="attrs" v-on="on" class="mr-1" icon small>
<!--            <v-avatar-->
...
<!--            </v-avatar>-->
    ...
    <v-main class="main-content">
      <v-container class="height-100pc" fluid>
        <transition appear name="slide-left">
          <router-view :key="routerKey" class="main-router-view" />
        </transition>
      </v-container>
    </v-main>
  </div>
...
export default {
    ...
    computed: {
      ...
      defaultAvatar() {
        ...
        //   String(userInfo ? 1 : userInfo.username),
        //   {}
        // ).toDataUrl();
        return userInfo;
      },
