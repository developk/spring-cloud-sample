        ...
        >
          <div class="login-block">
            ...
            <v-row align="end" class="mx-1">
              <v-spacer/>
              <v-btn color="#4cabef" dark tile width="100" @click="login"
              >로그인
              </v-btn
              >
            </v-row>
...
export default {
  data: () => ({
    ...
    text: "IMEPS",
    model: {
      username: DEFAULT_ACCOUNT.loginId,
  ...
  methods: {
    ...
    login() {
      this.$store
          ...
          .then(() => {
            //로그인 기록
              this.$store.dispatch("user/loginHistory")
                  .then(() => {
                    this.$router.push("/");
                  })
                  .catch((err) => {
  ...
  mounted() {
    ...
    if(store.state.user.username && !DEFAULT_ACCOUNT.loginId) {
      this.model.username = store.state.user.username;
    }
