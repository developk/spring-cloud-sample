...
import {saveUserProfile, updatePassword,} from "@/api/user";
...
export default {
  ...
  methods: {
    initBaseInfo() {
      ...
      this.usercard = GeoPattern.generate(String(this.userInfo.username), {}).toDataUrl();
      this.resetProfile();
      ...
      this.cardlines = [
        ...
        {
          icon: "mdi-update",
          text: `加入于 ${this.$moment(this.userInfo.createAt).format('YYYY-MM-DD')}`
        }
      ];
    },
    // initAccessLogs() {
    //   getAccessLogsByUserId(this.userInfo.username)
    //       .then((result) => {
